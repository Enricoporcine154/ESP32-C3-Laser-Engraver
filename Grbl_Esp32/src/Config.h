...
// ESP32-C3 是单核 RISC-V，只有 Core 0
// ESP32 原版是双核 Xtensa，有 Core 0 和 Core 1
#if defined(ESP32C3) || defined(ESP32_C3) || defined(CONFIG_IDF_TARGET_ESP32C3)
    #define SUPPORT_TASK_CORE 0  // ESP32-C3 单核，只能使用 Core 0
#else
    #define SUPPORT_TASK_CORE 1  // ESP32 双核，使用 Core 1 (Arduino 运行在 Core 1)
#endif
...
// ESP32-C3 不支持经典蓝牙(BluetoothSerial)，只支持BLE
// 因此在ESP32-C3上禁用蓝牙功能
#ifndef ESP32_C3
    #define ENABLE_BLUETOOTH  // enable bluetooth (仅原版ESP32支持)
#endif
...
// #define HOMING_INIT_LOCK  // 关闭上电强制归零锁定

// Number of homing cycles performed after when the machine initially jogs to limit switches.
// This help in preventing overshoot and should improve repeatability. This value should be one or
// greater.
// 设为0：每轴只需触发1次限位（方便手动测试）
// 设为1：每轴需触发2次限位（更精确，推荐有实际限位开关时使用）
static const uint8_t NHomingLocateCycle = 0;  // Integer (0-128), 0=单次触发
