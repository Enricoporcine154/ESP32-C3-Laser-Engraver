...
namespace Spindles {
    ...
    uint32_t PWM::set_rpm(uint32_t rpm) {
        ...
        // 调试输出已注释（太频繁）
        // grbl_msg_sendf(CLIENT_SERIAL, MsgLevel::Info, "set_rpm: rpm=%lu", rpm);
...
#ifdef ESP32_C3
        // ESP32-C3 使用 ledcWrite
        ledcWrite(_pwm_chan_num, duty);
#else
        // ESP32 直接操作寄存器，避免 ISR 兼容性问题
    ...
    /*
		...
		80,000,000 / freq = period (ESP32)
		40,000,000 / freq = period (ESP32-C3, uses different APB clock)
...
#ifdef ESP32_C3
        // ESP32-C3 uses 40MHz APB clock for LEDC
        uint32_t apb_clock = 40000000;
#else
        // ESP32 uses 80MHz APB clock for LEDC
        uint32_t apb_clock = 80000000;
#endif

        // increase the precision (bits) until it exceeds allow by frequency the max or is 16
        while ((1 << precision) < (uint32_t)(apb_clock / freq) && precision <= 16) {
            precision++;
        }
