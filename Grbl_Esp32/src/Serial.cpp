...
#if defined(ARDUINO_ESP32C3_DEV) || defined(ESP32C3) || defined(REVERT_TO_ARDUINO_SERIAL)
...
void client_init() {
    ...
    xTaskCreatePinnedToCore(heapCheckTask, "heapTask", 2000, NULL, 1, NULL, SUPPORT_TASK_CORE);
#endif

#if defined(ARDUINO_ESP32C3_DEV) || defined(ESP32C3)
    // ESP32-C3 使用原生USB CDC，不需要指定引脚
    Serial.begin(BAUD_RATE);
    client_reset_read_buffer(CLIENT_ALL);
    Serial.write("\r\n");  // create some white space after ESP32 boot info
#elif defined(REVERT_TO_ARDUINO_SERIAL)
...
#endif
#ifdef ESP32_C3
    // ESP32-C3 BLE 支持 - BLE数据已在handle中处理并放入inputBuffer
#endif
...
#ifdef ESP32_C3
        // ESP32-C3 BLE 处理
        WebUI::ble_config.handle();
        if (WebUI::ble_config.hasData()) {
            String bleData = WebUI::ble_config.readData();
            for (size_t i = 0; i < bleData.length(); i++) {
                uint8_t c = bleData[i];
                if (is_realtime_command(c)) {
                    execute_realtime_command(static_cast<Cmd>(c), CLIENT_BT);
                } else {
                    portENTER_CRITICAL(&myMutex);
                    client_buffer[CLIENT_BT].write(c);
                    portEXIT_CRITICAL(&myMutex);
                }
            }
        }
...
#ifdef ESP32_C3
    // ESP32-C3 BLE 输出
    if ((client == CLIENT_BT || client == CLIENT_ALL) && WebUI::ble_config.isConnected()) {
        WebUI::ble_config.sendData(text);
    }
#endif
